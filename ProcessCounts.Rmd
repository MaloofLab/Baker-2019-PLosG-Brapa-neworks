...
(Not working...)
```{r, eval=FALSE}
count.voom.int2 <- counts.voom.int[,(pdata$RIL=="RIL_234" | pdata$RIL=="RIL_311")] # these two lines are missing UN data and so the interaction terms cannot be estimated, and therfore we get an error unless we remove them.
voom.int.fit <- lmFit(counts.voom.int,design=design.int)
