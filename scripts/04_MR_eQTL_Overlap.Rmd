...
pl.UN
ggsave(str_c("../output/MR gene eQTL UN ", Sys.Date(), ".pdf"),width=12,height=8)
ggsave(str_c("../output/MR gene eQTL UN ", Sys.Date(), ".png"),width=10,height=5)
...
```{r}
overlaps <- sum(countOverlaps(qtl.ranges,eQTL.ranges)) #OK to ignore warnings

overlaps

mean(sim.results >= overlaps)

tibble(FVTQTL_vs_MReQTL_True_Overlaps=overlaps,
       N_Simulations_fewer_overlaps=sum(sim.results < overlaps),
       N_Simulations_greater_equal_overlaps=sum(sim.results >= overlaps),
       P_value=mean(sim.results >= overlaps)
) %>%
