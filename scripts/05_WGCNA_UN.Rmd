...
annotation  <- read.csv("../input/Brapa_V1.5_annotated.csv.gz")
```
...
cor.top
write.csv(cor.top,"../output/Eigen_trait_cor_UN_.5_threshold.csv")
```

write the Eigen genes
```{r}
head(MEs[,colnames(cor.top)])
write.csv(MEs[,colnames(cor.top)],"../output/Top_Eigen_genes_UN.csv")
```

write all Eigen genes

```{r}
write.csv(MEs,"../output/All_Eigen_genes_UN.csv")
```
...
save(annotation,blups.cor,blups.cor.5,blups.cor.P,blups.cor.sig,moduleColors,MEs,merge,expr.data.UN.t,
     file="../output/WGCNA_UN.Rdata")
```

## GO enrichment for each signficant cluster

```{r}
load("../output/WGCNA_UN.Rdata")
...
```{r}
file.remove("../output/SupTable_JM5_WGCNA_UN_GO.csv")
...
GO.results <- lapply(sig.modules, function(module) {
  ...
  write_csv(GO.out.tmp,path = "../output/SupTable_JM5_WGCNA_UN_GO.csv",append = TRUE)
