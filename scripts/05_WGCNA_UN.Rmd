...
```{r}
blups.cor <- cor(ME.blups[,2:11],ME.blups[,12:61])
range(blups.cor)
blups.cor
```
...
pdf("../output/Module-trait_heatmap_UN.pdf",height = 8,width = 12)
par(mar = c(8, 8, 3, 3));
# Display the correlation values within a heatmap plot
labeledHeatmap(Matrix = blups.cor,
               yLabels = names(blups.UN)[-1],
...
write.csv(MEs[,colnames(blups.cor.sig)],"../output/Sig_Eigen_genes_UN.csv")
```
