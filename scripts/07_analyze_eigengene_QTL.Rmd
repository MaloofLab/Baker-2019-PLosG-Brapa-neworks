...
bayesint.result <- as.tibble(bind_rows(bayesint.list,.id="trait")) %>% 
  ...
  group_by(trait,chr) %>% 
  summarize(eigen_eQTL_start=min(Mbp),eigen_eQTL_end=max(Mbp),min_eQTL_LOD=min(LOD),max_eQTL_LOD=max(LOD)) %>% 
  #for the high QTL peaks the interval width is 0.  That is overly precise and need to widen those.
  mutate(start=ifelse(eigen_eQTL_start==eigen_eQTL_end,max(0,eigen_eQTL_start-20000),eigen_eQTL_start), eigen_eQTL_end=ifelse(eigen_eQTL_start==eigen_eQTL_end,eigen_eQTL_end+20000,eigen_eQTL_end))
...
eigen.annotated <- lapply(1:nrow(bayesint.result),function(row) {
  ...
  results <- subset(BrapaAnnotation, chrom==qtl$chr &
                    start >= qtl$eigen_eQTL_start &
                    end <= qtl$eigen_eQTL_end)
}
)
...
eigen.annotated.small <- eigen.annotated %>% select(trait,eigen_eQTL_chrom=chrom, eigen_eQTL_start, eigen_eQTL_end, eigen_eQTL_candidate,ends_with("LOD"))
